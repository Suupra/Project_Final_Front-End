  ...
  <div class="login-box">
    ...
    <form>
      ...
      <a href="#">
        <span></span>
        <span></span>
        <p @click="register">Registar</p>
      </a>
    </form>
    <p>Já tem conta? <router-link to="/Sign-in" class="a2">Login!</router-link> <router-link to="/MainPage" class="a2">Home</router-link></p>
  </div>
</template>
...
const register = async () => {
  try {
    ...
    await setDoc(doc(db, 'users', user.uid), {
      uid: user.uid,
      email: user.email,
      displayName: user.displayName || '' // Se não houver um displayName, deixe-o como uma string vazia
    });
